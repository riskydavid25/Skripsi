...
void mqttCallback(char* topic, byte* payload, unsigned int length) {
  ...
  const char* fromID = doc["id"] | "";  // fallback jika tidak ada
  const char* type = doc["type"];
  bool status = doc["status"];

  if (!status && (String(type) == "call" || String(type) == "bill")) {
    if (String(fromID) == "ESP32_Receiver" || String(fromID) == "NodeRED" || String(fromID) == "") {
      Serial.println("🔄 Reset triggered via MQTT");
      resetSystem();
    } else {
      Serial.println("⚠️ Reset ignored: unauthorized sender ID");
    }
  }
}
...
void loop() {
  ...
  if (callState == LOW && lastCallState == HIGH) {
    ...
    sendMessage("call", true, callCount, timeStr);
    sendMessage("bill", false, billCount, timeStr);
    delay(200);
  }
  ...
  if (billState == LOW && lastBillState == HIGH) {
    ...
    sendMessage("call", false, callCount, timeStr);
    sendMessage("bill", true, billCount, timeStr);
    delay(200);
  }
